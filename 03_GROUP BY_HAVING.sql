...
SELECT DEPT_CODE , SUM(SALARY) -- 그룹별로 그룹함수(SUM) 수행
FROM EMPLOYEE
GROUP BY DEPT_CODE; --EMPLOYEE 테이블에서 DEPT_CODE 컬럼 값이 같은 행들끼리 묶음(그룹)
...
--  SELECT절에 명시한 컬럼을 조회할려면 컬럼 중
--  그룹함수가 적용되지 않은 컬럼을 
--  모두 GROUP BY절에 작성해야함.

-- EMPLOYEE 테이블에서 부서 별로 같은 직급인 사원의 급여 합계를 조회하고
-- 부서 코드 오름차순으로 정렬
SELECT DEPT_CODE "부서 코드", JOB_CODE "직급", SUM(SALARY) "급여 합계"
FROM EMPLOYEE
...
--SELECT DEPT_CODE "부서", SUM(SALARY)
--FROM EMPLOYEE
----WHERE SALARY>=3000000 -- 급여가 300A만 이상인 행(사원)만 조회
--GROUP BY DEPT_CODE
--ORDER BY DEPT_CODE; 

SELECT DEPT_CODE "부서", AVG(SALARY) "평균 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE "부서", SUM(SALARY) "급여 합계"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 직급코드 오름차순으로 정렬하세요
SELECT JOB_CODE "직급", COUNT(BONUS) "보너스를 받는 사원의 수"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
-- 부서 코드 오름차순으로 정렬하세요
SELECT  DEPT_CODE "부서", AVG(SALARY) "70년대생의 급여 평균"
FROM EMPLOYEE
...
-- UNION ALL : 여러개의 쿼리 결과를 하나로 합치는 연산자, UNION과의 차이점은 중복영역을 모두 포함시킨다. (합집합 +  교집합)
