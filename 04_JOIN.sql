...
/* JOIN 은 단순히 테이블을 두개를 붙이는것이 아닌 기준 삼은 테이블의 한 컬럼을 지정해 다른 테이블의 한 컬럼과 같은 행을 찾아
 * 기준 테이블 옆에 한행씩 붙여나감 */
...
-- 1) 연결에 사용할 두 컬럼명이 다른 경우 => JOIN 테이블명 ON
...
SELECT EMP_NO 사번, EMP_NAME 이름, JOB_CODE 직급코드, EMP_NAME 직급명
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); --> 컬럼 이름이 같아서 두 테이블의 같은 컬럼이 포개지듯 합쳐짐
...
SELECT EMP_NO 사번, EMP_NAME 이름, JOB_CODE 직급코드, EMP_NAME 직급명
FROM EMPLOYEE, JOB;
--WHERE JOB_CODE = JOB_CODE ; <-- 열의 정의가 애매합니다, 작성된 컬럼이 어떤 테이블의 컬럼인지 구분 불가


--=> 
SELECT EMP_NO 사번, EMP_NAME 이름, e.JOB_CODE 직급코드, EMP_NAME 직급명
...
--=> 하동운, 이오리가 누락
-- ㄴ> DEPT_CODE(NULL) 값과 같은 값이 DEPT_ID 컬럼에 존재하지 않아서 연결되지 않음, 최종 결과에서 제외
--   ㄴ=>  >INNER JOIN< 연결되지 않은 값은 결과에 미포함
...
--> JOIN 구문 기준으로 왼쪽에 작성된 테이블의 모든 행이 최종 결과(RESULT SET)에 포함 되도록 하는 JOIN
-- == LEFT JOIN
...
--  조인되는 테이블의 각 행들이 모두 매핑된 데이터가 검색되는 방법(곱집합)
	--> 모든 경우의 수를 보고싶을때 빼고는 직접 작성하는 경우가 없음
		--> 자연 조인 실패 결과로 교차 조인 결과 출력 (결과가 교차조인-> 자연조인 잘못됨)
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음 (똑같은 테이블이 2개가 있다고 생각!)

-- ANSI 표준
-- EMPLOYEE 테이블에서 각 사원의 이름, 사수 이름 조회
...
SELECT EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB; -- 알아서 같은 컬럼을 찾아서 JOIN


/* NATURAL JOIN 실패  -->  CROSS JOIN 결과 조회 */
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
NATURAL JOIN DEPARTMENT; -- 실패
...
JOIN LOCATION ON (DEPT_CODE = DEPT_ID);
...
/* 여러 테이블 조인 시 JOIN 순서를지키지 않아도 되는 경우*/
--> FROM에 있는 테이블과 JOIN 하는 경우

-- 이름, 직급명, 부서명 조회
SELECT EMP_NAME , EMP_NAME , DEPT_TITLE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_NAME 직급명, DEPT_CODE 부서명, LOCAL_NAME 근무지역명, SALARY 급여
FROM EMPLOYEE
	JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
	JOIN JOB USING(JOB_CODE)
	JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
-- 사원명, 주민번호, 부서명, 직급명을 조회하시오. EMPLOYEE, JOB, DEPT

-- ANSI
SELECT JOB_NAME 사원명, EMP_NO 주민번호, DEPT_TITLE 부서명, JOB_NAME 직급명
FROM EMPLOYEE
	JOIN JOB USING(JOB_CODE)
	JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-- ANSI
SELECT EMP_NAME 사원명, BONUS 보너스포인트, DEPT_TITLE 부서명, LOCAL_NAME 근무지지역명
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
SELECT EMP_NAME 사원명, JOB_NAME 직급명, SALARY 급여, 
	CASE 
		WHEN BONUS IS NOT NULL THEN SALARY *12*BONUS 
...
-- ANSI 		EMPLOYEE, DEPARTMENT, LOCATION L, NATIONAL N
...
-- SELF JOIN 사용
-- ANSI
SELECT E.EMP_NAME 사원명, E.DEPT_CODE 부서코드, EM.EMP_NAME 동료이름
FROM EMPLOYEE E
JOIN EMPLOYEE EM ON(E.DEPT_CODE = EM.DEPT_CODE)
AND E.EMP_ID != EM.EMP_ID
ORDER BY 1;

-- 오라클 전용
SELECT E.EMP_NAME 사원명, E.DEPT_CODE 부서코드, EM.EMP_NAME 동료이름
FROM EMPLOYEE E, EMPLOYEE EM
WHERE E.DEPT_CODE = EM.DEPT_CODE
AND E.EMP_ID != EM.EMP_ID
ORDER BY 1;
