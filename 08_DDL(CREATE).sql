...
-- 2) 컬럼명 지정 + 자료형 /* UTF-8 : 영어, 숫자, 기본 특수문자 == 1BYTE / 나머지 == 3BYTE */

-- 아이디 	: MEMBER_ID 	/ 자료형 : VARCAHR2(20)
...
-- ㄴ> DEFAULT (=기본값) : 컬럼의 기본값 지정 (필수 아님) ,
...
COMMENT ON COLUMN "MEMBER".MEMBER_NAME IS '회원 주민번호';
...
INSERT INTO "MEMBER" 
VALUES ('MEM01', '123ABC', '홍길동', '990102-1234567', TO_DATE('2024-03-06'));
...
--  USER_ID VARCHAR2(20) UNIQUE, 
    ...
    -- 테이블 레벨 --
--  UNIQUE (USER_NAME) --테이블 레벨 제약 조건 (이름X)
...
-- UNIQUE 복합키
-- 두 개 이상의 컬럼을 묶어서 하나의 UNIQUE 제약조건을 설정함, 테이블 레벨로만 지정이 가능
...
-- 한 개 컬럼에 설정할 수도 있고, 여러개의 컬럼을 묶어서 복합키로 설정할 수 있음
...
-- => PK 설정시 UNIQUE, NOT NULL 기능 자동 설정
---------------------------------------
...
-- PRIMARY KEY는 NULL이 들어갈 수 없음-> 복합키에서 중복문 모든 컬럼 묶어서 검사 , NOT NUJLL 은 각 컬럼별 검사
INSERT INTO USER_USED_PK2
...
-- ㄴ> 다른 테이블이 제공하는 컬럼에는 PK 또는 U 제약 조건이 설정 필요
...
SELECT * FROM USER_USED_FK LEFT JOIN USER_GRADE USING (GRADE_CODE);
...
--DELETE FROM USER_GRADE
--WHERE GRADE_CODE = 30; -- 실패, 자식테이블에서 기록, 사용 -> 삭제 불가
...
DELETE FROM USER_USED_FK3 WHERE GRADE_CODE = 10;
...
--> NOT NULL 제약조건을 제외한 다른 제약조건은 복사X
...
REFERENCES DEPARTMENT;  -- 생략시 자동으로 DEPARTMENT 테이블 PK 컬럼 참조
