...
-- <날짜 관련 함수>
-- SYSDATE : 현재 시간
-- SYSTIEMSTAMP : 현재 시간 (MS 포함, 표준 시간대)
SELECT SYSDATE, SYSTIMESTAMP  FROM DUAL;

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜 사이의 개월 수를 반환
SELECT MONTHS_BETWEEN('2024-01-28', SYSDATE) FROM DUAL;

-- ** ORACLE은 자료형이 맞지 않은 상황도 작성된 값의 형태가 요구하는 자료형의 형태를 띄고 있으면 자동으로 형변환(PARSING)을 진행한다.
-- 					ㄴ> EX) '2024-03-28' -> TO_DATE('2024-03-28', 'YYYYY-MM-DD')
...
-- MONTH_BETWEEN()은 윤년(2월 29일이 포함된 해) 계산이 자동으로 수행
--> YEAR, MONTH 단위 계산시 더 정확한 값을 얻어낼 수 있다
...
/* 숫자 -> 문자열 변환시 문제 상황*/
-- ㄴ> 포맷에 지정된 칸 수가 숫자 길이보다 적으면 전부 #으로 변환 출력
...
-- /, (), :, - 는 일반적으로 날짜 표기시 사용하는 기호 <- 패턴으로 인식되어 오류 발생 X

-- 년, 월, 일 같은 한글 또는 날짜와 관련 없는 문자는 패턴 X, 오류 발생
--	ㄴ>  "" 감싸면 사용 가능 ("" 의미 : 있는 그대로 인식)
...
FROM DUAL;
-- ㄴ> 문자열이 날짜를 표현하는 형식이면 포맷 지정 없이 바로 변경 가능

SELECT TO_DATE('04-03-2024')
FROM DUAL;
-- ㄴ> SQL Error [1830] [22008]: ORA-01830: 날짜 형식의 지정에 불필요한 데이터가 포함되어 있습니다 <- 일반적으로 사용하는 형태X, 인식 불가 오류

SELECT TO_DATE('04-03-2024', 'DD-MM-YYYY')
FROM DUAL; 
-- ㄴ> 형식 지정시 인식 가능

SELECT TO_DATE('02월 27일 화요일 12시 15분', 'MM"월" DD"일" DAY HH"시" MI"분"')
FROM DUAL;
...
SELECT EMP_NAME , JOB_CODE , SALARY,
	DECODE(JOB_CODE, 'J7', SALARY *1.1, 'J6', SALARY *1.5, 'J5', SALARY *1.2, SALARY *1.05) 지급급여
FROM EMPLOYEE;
...
-- SELECT 절에 작성된 컬럼이 아니라고해서 ORDER BY 절에 사용 못하는게 아님
			--> FROM 절에서 해석되어 사용 가능한 상태
...
/**/ SELECT SUM(SALARY)					-- 3) SALARY 컬럼 값의 합을 조회
...
SELECT SALARY, FLOOR( AVG(SALARY) )
FROM EMPLOYEE;
-- ㄴ> SQL Error [937] [42000]: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다, 23행과 1행 조회 - 오류
...
-- 모든 사원 중 가장 빠른 입사일, 최근 입사일 이름 오름차순에서 제일 먼저 작성되는 이름, 마지막에 작성되는 이름 조회
...
-- 전화번호가 있는 사원의 수2 <- NULL이 아닌 행의 수만 카운트
SELECT COUNT(PHONE)
FROM EMPLOYEE;
...
FROM EMPLOYEE;
-- ㄴ> COUNT() 내에 컬럼명 작성시 NULL 제외되어 개수를 7이 아닌 6 반환
...
-- EMPLOYEE 테이블에 존재하는, 남자 사원의 수
SELECT COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), '2', NULL, '남자')) 남자
FROM EMPLOYEE;
